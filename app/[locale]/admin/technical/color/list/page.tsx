...
import { Color, Prisma } from '@/prisma/app/generated/prisma';
...
import { exportColors } from '@/lib/actions/technical/color.action';
import { deleteSelectedColors } from '@/components/admin/DeleteSelectedButton';
...
export default async function ColorListPage({ searchParams }: PageProps) {
    ...
    const query: Prisma.ColorWhereInput = {};
    if (queryParams) {
    ...
    const orderBy: Prisma.ColorOrderByWithRelationInput[] = sortValues.map((sortValue) => {
    ...
    const [data, count] = await prisma.$transaction([
        ...
        prisma.color.count({ where: query }),
    ]);
    ...
    const columns = [
        ...
        { header: t('hex'), accessor: 'hex', className: '' },
    ];

    const renderRow = (item: ColorList) => (
        <tr key={item.id} className="border-b border-slate-100 text-sm hover:bg-gradient-more-lighter">
            <td>
                <Checkbox id={item.id} />
            </td>
            <td className="py-2">{item.name}</td>
            <td className="py-2">{item.hex}</td>
            <td className="py-2">
                <div className="flex items-center gap-2">
                    <FormContainer table="color" type="update" data={item} />
                    <FormContainer table="color" type="delete" id={item.id} />
                </div>
            </td>
        </tr>
    );
        ...
        <>
            ...
            <div className="bg-white p-4 rounded-md flex-1 mt-0">
                <div className="flex items-center lg:justify-between justify-start">
                    <h1 className="hidden lg:block text-lg font-semibold">{t('allColors')}</h1>
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4 w-full md:w-auto">
                        ...
                        <div className="flex flex-wrap items-center gap-4 lg:self-end">
                            ...
                            <FilterTechnicalDropdown
                                ...
                                entityName={t('color')}
                            />
                            <ExportButton exportAction={exportColors} entityName={t('color')} />
                            <ReloadButton />
                            <DeleteSelectedButtonClient deleteAction={deleteSelectedColors} entityName={t('color')} />
                            <FormContainer table="color" type="create" />
                        </div>
                    </div>
                </div>
