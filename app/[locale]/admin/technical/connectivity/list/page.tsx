...
import { Connectivity, Prisma } from '@/prisma/app/generated/prisma';
...
import { exportConnectivities } from '@/lib/actions/technical/connectivity.action';
import { deleteSelectedConnectivities } from '@/components/admin/DeleteSelectedButton';
...
export default async function ConnectivityListPage({
    ...
    const query: Prisma.ConnectivityWhereInput = {};
    if (queryParams) {
    ...
    const orderBy: Prisma.ConnectivityOrderByWithRelationInput[] = sortValues.map((sortValue) => {
    ...
    const [data, count] = await prisma.$transaction([
        ...
        prisma.connectivity.count({ where: query }),
    ]);
    ...
    const renderRow = (item: ConnectivityList) => (
        <tr key={item.id} className="border-b border-slate-100 text-sm hover:bg-gradient-more-lighter">
            ...
            <td className="py-2">
                <div className="flex items-center gap-2">
                    <FormContainer table="connectivity" type="update" data={item} />
                    <FormContainer table="connectivity" type="delete" id={item.id} />
                </div>
            </td>
        </tr>
    );
        ...
        <>
            ...
            <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
                <div className="flex items-center justify-between">
                    <h1 className="hidden md:block text-lg font-semibold">{t('allConnectivities')}</h1>
                    <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                        ...
                        <div className="flex items-center gap-4 self-end">
                            ...
                            <FilterTechnicalDropdown
                                ...
                                entityName={t('connectivity')}
                            />
                            <ExportButton exportAction={exportConnectivities} entityName={t('connectivity')} />
                            <ReloadButton />
                            <FormContainer table="connectivity" type="create" />
                            <DeleteSelectedButtonClient
                                deleteAction={deleteSelectedConnectivities}
                                entityName={t('connectivity')}
                            />
                        </div>
                    </div>
