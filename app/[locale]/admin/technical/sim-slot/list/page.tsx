...
import { Prisma, SimSlot } from '@/prisma/app/generated/prisma';
...
import { exportSimSlots } from '@/lib/actions/technical/sim-slot.action';
import { deleteSelectedSimSlots } from '@/components/admin/DeleteSelectedButton';
...
type SimSlotList = SimSlot;

export default async function SimSlotListPage({ searchParams }: PageProps) {
    const t = await getTranslations('SimSlotList');
    ...
    const query: Prisma.SimSlotWhereInput = {};
    if (queryParams) {
    ...
    const orderBy: Prisma.SimSlotOrderByWithRelationInput[] = sortValues.map((sortValue) => {
    ...
    const [data, count] = await prisma.$transaction([
        prisma.simSlot.findMany({
            where: query,
            take: ITEM_PER_PAGE,
            skip: ITEM_PER_PAGE * (p - 1),
            orderBy,
        }),
        prisma.simSlot.count({ where: query }),
    ]);

    // Define columns after data is initialized
    const columns = [
        { header: <CheckboxHeader itemIds={data.map((item: any) => item.id)} />, accessor: 'check' },
        { header: t('title'), accessor: 'title', className: '' },
    ];

    const renderRow = (item: SimSlotList) => (
        <tr key={item.id} className="border-b border-slate-100 text-sm hover:bg-gradient-more-lighter">
            ...
            <td className="py-2">
                <div className="flex items-center gap-2">
                    <FormContainer table="simSlot" type="update" data={item} />
                    <FormContainer table="simSlot" type="delete" id={item.id} />
                </div>
            </td>
        </tr>
    );
        ...
        <>
            ...
            <div className="bg-white p-4 rounded-md flex-1 mt-0">
                <div className="flex items-center lg:justify-between justify-start">
                    <h1 className="hidden lg:block text-lg font-semibold">{t('allSimSlots')}</h1>
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4 w-full md:w-auto">
                        ...
                        <div className="flex flex-wrap items-center gap-4 lg:self-end">
                            ...
                            <FilterTechnicalDropdown
                                ...
                                entityName={t('simSlot')}
                            />
                            <ExportButton exportAction={exportSimSlots} entityName={t('simSlot')} />
                            <ReloadButton />
                            <DeleteSelectedButtonClient
                                deleteAction={deleteSelectedSimSlots}
                                entityName={t('simSlot')}
                            />
                            <FormContainer table="simSlot" type="create" />
                        </div>
                    </div>
                </div>
